...
| **Settings:** library folder, YouTube login, MusicBrainz, token helper, parallel requests — and what it found: config file, JS runtime, token generator. | **Library, filtered to one artist:** covers, track counts, MusicBrainz badge, and "check for new albums" for that artist alone. Press <kbd>/</kbd> to filter the library — by album, artist **or song**. |
